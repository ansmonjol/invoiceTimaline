...
export function Query(_fnNameS, _aliasS_OR_Filter){
    ...
    this.props = function(findA) { // THIS NEED TO BE A "FUNCTION" to scope 'arguments'
        if( ! findA){
...
Query.prototype = {
    ...
    toString : function(){
        ...
        const str = `${ (this.aliasS) ? (this.aliasS + ":") : "" } ${this.fnNameS } ${ (0 < this.headA.length)?"("+this.headA.join(",")+")":"" } ${ bodyS }`;
        return str;
    },
    graph: function(){
